...
Testing city counts and removing Tulsa, AL for having only 1 observation:
...
Creating a table with the number of resolved and unresolved homicides for each of the cities present within the database:
...
Creating the function that will run `prop.test` for the `homicide_raw` dataframe:
...
2. Iterating through the unique `city_state` names in the `homicide_proptest` function using `purrr::map_dfr`
